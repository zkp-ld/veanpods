...
const _builtinContexts: Array<[Url, JsonLd]> = [
  ...
  ["https://www.w3.org/2018/credentials/examples/v1", vcExampleContext as JsonLd],
...
const documents = new Map<string, JsonLd>(_builtinDIDDocs.concat(_builtinContexts));

const customDocLoader =
  (url: string): RemoteDocument => {
...
export const customLoader: (url: string) => Promise<RemoteDocument> = jsonldSignatures.extendContextLoader(customDocLoader);